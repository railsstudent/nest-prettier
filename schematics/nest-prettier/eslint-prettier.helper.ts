...
export function addEslintPrettier(options: Schema): Rule {
  return (tree: Tree, context: SchematicContext) => {
    ...
    if (options.eslintFileFormat === ESLINT_FILE_FORMAT.PACKAGE_JSON) {
      ...
      context.logger.info(
        `Append plugin and rule from ${eslintTemplate} to ${configFileName}. Then, delete the template file.`,
      );
...
function updateEslintConfig(
  ...
  prettierRule: (string | PrettierOptions)[],
) {
